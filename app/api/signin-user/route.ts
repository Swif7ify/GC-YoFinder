...
import { UserLogin } from "@/server/handlers/UserAuth";
...
const validateField = async (
	...
	if (typeof remember_me !== "boolean") return false;
...
export async function POST(request: NextRequest) {
	try {
		...
		const isValid = await validateField(email, password, remember_me);
		if (!isValid)
		...
		const response = await UserLogin(email, password, remember_me);
...
async function setAuthRememberCookies(
	cookieStore: any,
	selector: string,
	token: string
) {
